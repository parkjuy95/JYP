...
public class poisonmagic : MonoBehaviour
{
    ...
    public float damage;
    public float poison;
    public float poisondottime;
    public static float meeledamage;
    ...
    void attack()
    {
        ...
        if (test == true)
        {
            test.GetComponent<poisonspeed>().targetPosition = (Target.transform.position);
            test.GetComponent<poisonspeed>().target = (Target);
        }
        else
        {
            Destroy(test);
