...
public class onetargetsmagic : MonoBehaviour
{
    public float Range;
    ...
    public float damage;
    public static float onetargetsmagicdamage;
    // Start is called before the first frame update
    void Start()
    {
        onetargetsmagicdamage = damage;
    ...
    void attack()
    {
        counttime += Time.deltaTime;
            if (Target != null && counttime > cooltime)
            {
                ...
                aBullet.GetComponent<onetargetspeed>().targetPosition = (Target.transform.position - transform.position).normalized;
               // aBullet.transform.localScale = new Vector3(20f, 20f);                                                       //크기
